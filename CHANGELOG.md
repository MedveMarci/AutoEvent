...
- CedMod is no longer a hard dependency: its FF autoban is now toggled via reflection, so AutoEvent works with or without CedMod installed.
...
- Among Us:
   ...
   - A late-joining (non-participant) player pressing the meeting button could freeze the game in a meeting that never started.
   - Body reports no longer remove SilentWalk from everyone; meetings now consistently skip dead players when teleporting.
   ...
   - The MedBay scan (Submit Scan) task is now actually assigned to crewmates - previously the scanner existed but the task was never generated.
