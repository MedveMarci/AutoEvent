...
public class Translation : EventTranslation
{
    ...
    public string StartPrisoners { get; set; } =
    ...
    public string PrisonersWin { get; set; } =
        "<color=red><b><i>Prisoners Win</i></b></color>\n<color=red>{time}</color>";

    public string JailersWin { get; set; } =
        "<color=#14AAF5><b><i>Jailers Win</i></b></color>\n<color=red>{time}</color>";

    public string LockdownOnCooldown { get; set; } =
        "You cannot trigger a lockdown, while it is on cooldown. Cooldown Remaining: {cooldown}";
